      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 1 }}>
        <div className="mx-auto w-[80%]">
          <div className=" text- mb-8 flex items-center text-sm">
            ...
            <p className=" uppercase">{singleBooking?.approvalStatus}</p>
          </div>
          <>
            <RentersInformation firstName={singleBooking?.user?.firstName} lastName={singleBooking.user?.lastName} />
            <RentersBookingPeriod
              startPeriod={singleBooking?.startDate?.split("T")[0]}
              endPeriod={singleBooking?.endDate?.split("T")[0]}
            />
            <RentersAdditionalServices
              delivery={singleBooking?.services?.map(({ delivery }) => delivery)}
              packing={singleBooking?.services?.map(({ parking }) => parking)}
            />
          ...
          <DisapproveBookingModal id={singleBooking?._id} />
        </div>
      </motion.div>
    </DashboardLayout>
  );
